...
def process_one_seq(seq_name):
    ...
    command = f'python reader.py --filename {raw_data_dir}/{seq_name}/{seq_name}.sens --output_path {target_seq_dir} --export_intrinsics'
    os.system(command)

    # copy the point cloud file here
    os.system(f'cp {raw_data_dir}/{seq_name}/{seq_name}_vh_clean_2.ply {target_seq_dir}')
